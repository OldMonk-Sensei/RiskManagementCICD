...
//     })
// });
//Import the CDS facade Object
const cds = require('@sap/cds')

// The service implementation with all service handlers
module.exports = cds.service.impl(async function () {
    // Define constants for the Risk and Business Partner entities from the risk-service.cds file
    const { Risks, BusinessPartners } = this.entities;
    //Handler for After read operation on Risks
    this.after("READ", Risks, (data) => {
        //Convert to Array
        const risks = Array.isArray(data) ? data : [data];
        risks.forEach((risk) => {
    ...
    /**
     * Event handler for the read -event of Business Partner entity
     * Each request to Business Partner require API key in Header
     */
